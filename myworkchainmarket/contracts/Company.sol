...
contract Company {
    ...
    /*
    function companyList() public view returns (address[], uint[], string[] memory, string[] memory, uint[], uint[], bool[]) {
        address[] memory companyIds = new address[](companyList.length);
        uint[] memory indexs = new uint[](companyList.length);
        string[] memory names = new string[](companyList.length);
        string[] memory locations = new string[](companyList.length);
        uint[] memory latitudes = new uint[](companyList.length);
        uint[] memory longitudes = new uint[](companyList.length);
        bool[] memory useYns = new bool[](companyList.length);
        
        for (uint i = 0; i < companyList.length; i++) {
            address companyId = companyList[i];
            CompanyEntry storage entry = companyMap[companyId];
            companyIds[i] = companyId;
            indexs[i] = entry.index;
            names[i] = entry.name;
            locations[i] = entry.location;
            latitudes[i] = entry.latitude;
            longitudes[i] = entry.longitude;
            useYns[i] = entry.useYn;
            
        }
        
        return (companyIds, indexs, names, locations, latitudes, longitudes, useYns);
    }
    */
}
